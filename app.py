...
ms_logo = open("ms_logo.png", "rb")
ms_logo_contents = ms_logo.read()
ms_logo_b64 = base64.b64encode(ms_logo_contents).decode("utf-8")
ms_logo.close()
...
if Certified_flow == "Bulk Mapping":
    ...
    if button and uploaded_file is not None and st.session_state.file_path is None:
        ...
        required_columns = {"Lead ID", "jobtitle", "LS Title", "LS Company", "LS Lead Job Functions", "LS Company Industry", "LS Lead Department"
        }
    ...
    col1.divider()
    if st.session_state.file_path and st.session_state.progress_status == False:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers = 5) as executor:
                    ...
                    for index, row in batch.iterrows():
                        job_entry = ({
                            ...
                            for col in ['Lead ID', 'jobtitle', 'LS Title', 'LS Company', 'LS Lead Job Functions', 'LS Company Industry', 'LS Lead Department']
                        })

                    # futures = {executor.submit(job_role_agent.run, title): title for title in batch}
                    # futures.update({executor.submit(job_role_agent.run, job_entry): job_entry['jobtitle']})
                        futures[executor.submit(job_role_agent.run, job_entry)] = job_entry
                        # print(" ====== job entry: ", job_entry)
    ...
    if st.session_state.progress_status:
        ...
        confidence_threshold = col10.slider("Confidence Score",min_value = 0.0,max_value = 1.0,step = 0.1)
        ...
        with open(validated_file_path, "rb") as file:
            btn = col12.download_button(label = "Download Certified Mapping",data = file,file_name = "validated_mapping_results.csv")
else:
    ...
    if st.session_state.result_dict :
        ...
        st.session_state.result_df = st.session_state.result_df[['input_job_title',"detected_language","Status","matched_standard_role","marketing_audience","function","seniority","confidence_score"]]
        ...
        st.data_editor(df_renamed, key = "table_editor", num_rows = "dynamic", disabled = ["Job Title","Language","Valid JT", "Job Role", "Function", "Seniority", "Marketing Audience"], hide_index = True,width = 1800)
