...
if Certified_flow == "Single Mapping":
    ...
    job_role_agent = MapperAgent()
    job_entry = {
        # Favor Job Title if provided, else fallback to LS Title
        'jobtitle': input_job_title.strip() if input_job_title.strip() else ls_title.strip(),
        'LS Title': ls_title,
    ...
    if button1 and (input_job_title.strip() or ls_title.strip()):
    ...
    if st.session_state.result_dict:
        ...
        display_columns = [
            'Lead ID', 'input_job_title', 'detected_language', 'Status',
            'matched_standard_role', 'marketing_audience', 'function',
            'seniority', 'confidence_score'
        ]
        result_df_cleaned = st.session_state.result_df[display_columns].copy()

        df_renamed = result_df_cleaned.rename(columns={
